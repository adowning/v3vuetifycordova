    ...
    <v-layout align-center column fill-height mt-3>
      ...
      <v-dialog v-model="clockOut" max-width="290">
        <v-card>
          ...
          <v-card-text>You will need to clock out from another device first in order to clock in on this one.</v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
import CloudApi from "../../services/cloudboost";
...
export default {
  ...
  data() {
    return {
      ...
      fbLoading: false,
      error: "",
      andrews_logo: "../../assets/andrews_logo.png",
      hasScroll: false,
      isUnderstood: true,
      dontShowAgain: false,
      isShowLogin: false,
  ...
  methods: {
    ...
    login(result) {
      ...
      //     },
      //     error => {
      //       dialogs.message("Email or password incorrect.", {
      //         duration: 10,
      //         estado: "error"
      //       });
      //       if (error) {
