...
function onClockIn(store, user) {
  store.commit(types.DEVICE_ADD_EMPLOYEELIST, user)
  // fetchUser(store)
}
...
function clockIn(store, creds) {
  return authApi.clockIn(creds.humanityId).then(
    response => {
      // onClockIn(store, response.user)
      // store.commit(types.DEVICE_ADD_USER_USERLIST, employeeId)
      console.log(response.status)
      ...
      // store.commit(types.AUTH_SET_TOKEN, token)
      store.dispatch('device/addEmployee', creds.employeeId, { root: true })
...
function clockOut(store, creds) {
  return authApi.clockOut(creds.humanityId).then(
    response => {
      ...
      store.dispatch('device/removeEmployee', creds.employeeId, { root: true })
      console.log(response)
